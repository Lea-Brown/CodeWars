...
IMPORTANT. This was written but the openai chatbot I was testing. Is this the future???
...
function revrot(str, sz) {
    ...
    for (var i = 0; i < str.length; i += sz) {
        var chunk = str.substring(i, i + sz);
        if (chunk.length < sz) {
            break;
        }
        chunks.push(chunk);
    ...
    for (var chunk of chunks) {
        if (chunk.split("").map(c => parseInt(c) ** 3).reduce((a, b) => a + b) % 2 == 0) {
